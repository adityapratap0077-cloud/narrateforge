...
invocation contains `--narrated` (or the legacy `--voice` alias), set
`voice.enabled = true`. Enable narration only for that run. Do not enable
narration automatically and do not fall back to the normal no-voice workflow.
Full pipeline: [references/narrated.md](references/narrated.md).

**Standing user preference (2026-09-24):** a plain `/brag` run stays plain —
no voiceover, no burned-in captions, no smart audio ducking. The "improved"
upgrades (voiceover, timed captions, ducked music) are opt-in only: add them
solely when the user explicitly asks for narration (or passes `--narrated` /
`--voice`) in that run. Never bundle them into a default run.
...
When narrated mode (`--narrated` / `--voice`) is enabled, follow
[references/narrated.md](references/narrated.md) end to end. Write narration
that complements the visuals, does not simply read visible text, matches
scene pacing, sounds natural and conversational, and moves smoothly between
scenes. Keep the script concise and specific to the product so the voice
feels like part of the edit rather than a separate narration track.
...
When `--narrated` is on, also write a `Narration` section: one timed
narration line per scene, vox-style (cold open → stakes → turn → kicker),
plain spoken text. See [references/narrated.md](references/narrated.md).
