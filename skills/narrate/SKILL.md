---
name: narrate
description: Make a narrated promo video (voiceover) for a website or project. Use when the user says "/narrate", "narrate this", or wants a video with voiceover. Defaults to the brag-style narrated cut; "/narrate flick" uses the Remotion scene-animation style.
---

# /narrate

One easy command for narrated videos. This command always narrates — voiceover over the video, no captions.
...
2. Run that skill's narrated mode (`--narrated`) end to end: narration script → TTS per line → fit-check each take against its scene duration → ducked music bed → final mp4.
